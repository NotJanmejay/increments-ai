...
def upload_pdf(request, id):
    ...
    pdf_embedding = PDFEmbedding(file_name=pdf_file.name, file=pdf_file, teacher_id = teacher.id, vector_store_id = vector_store_id)
    pdf_embedding.save()
...
def check_embedding_status(request, file_name):
    ...
    if not status_info:
        return Response({"error": f"No embedding process started for {file_name}."}, status=status.HTTP_404_NOT_FOUND)

    # Determine the status code based on the status of the embedding process
    status_code = (
        status.HTTP_202_ACCEPTED if status_info.get("status") == "processing" 
